        ...
        uchar3 * outPixels)
{
	...
	if (r < height && c < width){
		...
		for (int filterR = 0; filterR < filterWidth; filterR++){
			for(int filterC = 0; filterC < filterWidth; filterC++){
				uchar3 s_inPixel = s_inPixels[(threadIdx.y + filterR) * sharedWidth + (threadIdx.x + filterC)];
				float filterVal = filter[filterR * filterWidth + filterC];
				outPixel.x += s_inPixel.x * filterVal; 
				outPixel.y += s_inPixel.y * filterVal; 
				outPixel.z += s_inPixel.z * filterVal; 
			}
		}
		outPixels[r * width + c] = make_uchar3(outPixel.x, outPixel.y, outPixel.z);	
	}
        ...
        bool useDevice=false, dim3 blockSize=dim3(1, 1), int kernelType=1)
{
	...
	else // Use device
	{
		...
		int s_inPixelsSize = ((blockSize.x + filterWidth - 1) * (blockSize.y + filterWidth - 1)) * sizeof(uchar3);
		timer.Start();
		if (kernelType == 1)
		{
		...
		else if (kernelType == 2)
		{
			...
			blurImgKernel2<<<gridSize, blockSize, s_inPixelsSize>>>(d_inPixels, width, height, d_filter, filterWidth,d_outPixels);

		}
		else
		{
			// TODO: call blurImgKernel3
			blurImgKernel3<<<gridSize, blockSize, s_inPixelsSize>>>(d_inPixels,  width,  height, filterWidth, d_outPixels);
		}
		timer.Stop();
