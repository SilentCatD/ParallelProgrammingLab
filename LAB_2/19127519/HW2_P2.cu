        ...
        bool useDevice=false, dim3 blockSize=dim3(1), int nStreams=1)
{
	...
	else // Use device
	{
        ...
        int streamSize = (n + nStreams - 1)/ nStreams;
        for(int i = 0; i < nStreams; i++){
            int offset = i * streamSize;
            int offSetSize = streamSize;
            if(i==nStreams-1){
                offSetSize = n - offset;
            }
            dim3 gridSize((offSetSize-1)/ blockSize.x + 1);
            CHECK(cudaMemcpyAsync(&d_in1[offset], &in1[offset], offSetSize * sizeof(int), cudaMemcpyHostToDevice, streams[i]));
            CHECK(cudaMemcpyAsync(&d_in2[offset], &in2[offset], offSetSize * sizeof(int), cudaMemcpyHostToDevice, streams[i]));
            addVecKernel<<<gridSize , blockSize, 0, streams[i]>>>(&d_in1[offset], &d_in2[offset], offSetSize, &d_out[offset]);
            CHECK(cudaMemcpyAsync(&out[offset], &d_out[offset], offSetSize * sizeof(int), cudaMemcpyDeviceToHost, streams[i]));
